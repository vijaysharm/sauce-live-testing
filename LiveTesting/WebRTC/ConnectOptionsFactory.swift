...
//			}
			...
			builder.areInsightsEnabled = false // self.appSettingsStore.areInsightsEnabled
			...
			builder.bandwidthProfileOptions = BandwidthProfileOptions(
				videoOptions: VideoBandwidthProfileOptions { builder in
					builder.mode = TwilioVideo.BandwidthProfileMode(
						setting: BandwidthProfileMode.collaboration // self.appSettingsStore.bandwidthProfileMode
					)
					builder.maxSubscriptionBitrate = nil // self.appSettingsStore.maxSubscriptionBitrate as NSNumber?
					builder.dominantSpeakerPriority = Track.Priority(
						setting: TrackPriority.serverDefault // self.appSettingsStore.dominantSpeakerPriority
					)
					builder.trackSwitchOffMode = Track.SwitchOffMode(
						setting: TrackSwitchOffMode.serverDefault//self.appSettingsStore.trackSwitchOffMode
					)
					builder.clientTrackSwitchOffControl = TwilioVideo.ClientTrackSwitchOffControl(
						setting: ClientTrackSwitchOffControl.sdkDefault//self.appSettingsStore.clientTrackSwitchOffControl
					)
					builder.contentPreferencesMode = TwilioVideo.VideoContentPreferencesMode(
						setting: VideoContentPreferencesMode.sdkDefault//self.appSettingsStore.videoContentPreferencesMode
					)
				}
			)
...
//			}
//			builder.encodingParameters = EncodingParameters(audioBitrate: 16, videoBitrate: 1_200)

			if false /* self.appSettingsStore.isTURNMediaRelayOn */ {
